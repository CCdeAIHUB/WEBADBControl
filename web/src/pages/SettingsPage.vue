...
import { AlertTriangle, Bot, KeyRound, Monitor, Plus, Save, Server, ShieldCheck, Trash2 } from 'lucide-vue-next'
...
async function save() {
  try {
    ...
    ui.notify('设置已保存', '新的配置已立即生效。', 'success')
  } catch (error) {
    ui.failure(toAppError(error))
  }
}
...
async function changeAdminPassword() {
  ...
  try {
    ...
    ui.notify('管理密码已更新', '其他浏览器中的旧会话已失效。', 'success')
  } catch (error) {
  ...
  <div class="mx-auto grid max-w-5xl gap-4">
    <section class="card p-5" :class="mustChangePassword ? '!border-amber-300 bg-amber-50/60 dark:!border-amber-500/30 dark:bg-amber-500/5' : ''">
      <div class="flex items-start gap-3">
        ...
        <div><h2 class="section-title m-0">管理密码</h2><p class="mt-1 text-xs leading-5" :class="mustChangePassword ? 'text-amber-700 dark:text-amber-300' : 'text-slate-400'">{{ mustChangePassword ? '当前仍在使用默认密码 admin，请先修改密码后再进入其他功能。' : '修改登录管理后台所使用的密码，修改后其他旧会话会立即失效。' }}</p></div>
      </div>
      <div class="mt-5 grid gap-4 md:grid-cols-3">
        ...
        <label class="text-xs font-medium">新密码<input v-model="newPassword" type="password" autocomplete="new-password" minlength="8" maxlength="128" class="field mt-2" placeholder="至少 8 个字符" /></label>
