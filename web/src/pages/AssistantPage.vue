...
<template>
  ...
  <div class="grid min-h-[calc(100vh-190px)] gap-4 xl:grid-cols-[1fr_280px]">
    ...
    <aside class="space-y-4"><div class="card p-4"><h3 class="section-title m-0">对话配置</h3><label class="mt-4 block text-xs font-medium">AI 模型</label><select v-model="modelId" class="field mt-2"><option v-if="!models.length" value="">请先添加模型</option><option v-for="model in models" :key="model.id" :value="model.id">{{ model.name }}</option></select><p v-if="selectedModel" class="mt-2 text-[10px] text-slate-400">{{ selectedModel.model }} · {{ selectedModel.vision ? '支持视觉' : '仅文本' }}</p><label class="mt-4 block text-xs font-medium">目标设备</label><select v-model="deviceId" class="field mt-2"><option value="">暂不指定</option><option v-for="device in devices.online" :key="device.id" :value="device.id">{{ device.name }}</option></select></div><div class="card p-4 text-xs leading-6 text-slate-500 dark:text-slate-400"><div class="mb-2 flex items-center gap-2 font-semibold text-slate-800 dark:text-slate-200"><Bot :size="15" class="text-brand-600" />安全边界</div>屏幕只在你明确附加时发送。敏感或破坏性设备操作仍需二次确认，不会由普通对话自动执行。</div></aside>
  </div>
</template>
