...
async function createQR() {
  ...
  try {
    qrSession.value = await createQRPairing()
  } catch (error) {
    ui.failure(toAppError(error))
  } finally {
    phase.value = 'idle'
...
async function submit() {
  if (mode.value === 'qr') {
    if (!qrSession.value || busy.value) return
    phase.value = 'qrPairing'
    try {
      const result = await pairQRDevice(qrSession.value.sessionId)
  ...
  <Teleport to="body">
    <div v-if="open" class="fixed inset-0 z-[80] grid place-items-center bg-black/35 p-4 backdrop-blur-[2px]" @click.self="emit('close')">
      <section class="surface w-full max-w-lg rounded-xl p-5 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="wireless-dialog-title">
        ...
        <div class="mt-6 flex justify-end gap-2"><button class="btn-secondary" @click="emit('close')">取消</button><button class="btn-primary" :disabled="busy || (mode === 'qr' ? !qrSession : !endpoint.trim() || (mode === 'pair' && pairingCode.length !== 6))" @click="submit">{{ phase === 'creatingQr' ? '正在生成…' : phase === 'qrPairing' ? '正在确认配对…' : phase === 'pairing' ? '正在配对…' : phase === 'connecting' ? '正在连接…' : mode === 'qr' ? '已扫码，开始配对' : mode === 'pair' ? '配对设备' : '连接并验证' }}</button></div>
      </section>
    </div>
  </Teleport>
</template>
